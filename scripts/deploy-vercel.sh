...
if [ ! -f .env.local ]; then
  echo "Error: .env.local wajib ada. Isi TURSO_*, SESSION_SECRET, ADMIN_*."
  exit 1
fi
...
REQUIRED=(TURSO_DATABASE_URL TURSO_AUTH_TOKEN SESSION_SECRET ADMIN_EMAIL ADMIN_PASSWORD)
...
echo "==> Sync 5 env vars ke Vercel (production + preview + development)"
for k in "${REQUIRED[@]}"; do
